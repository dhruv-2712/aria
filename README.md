...
```
ARIA — Autonomous Research & Intelligence Architecture
Python · Groq (Llama 3.1) · FastAPI · SQLite

• Architected an 8-agent autonomous AI system with role specialisation,
  adversarial critique loops, and cross-domain synthesis pipelines

• Implemented an adversarial feedback mechanism between Analyst and
  Devil's Advocate agents, improving output claim accuracy through
  iterative revision (triggers when weak claims exceed 30% threshold)

• Built a dynamic orchestration layer that routes tasks, handles failures
  with a retry queue, and manages global state across all agents

• Designed a research gap-detection loop where the Classifier identifies
  under-covered domains and automatically triggers targeted re-research

• Generated 3-format reports (executive/standard/technical) in parallel
  using ThreadPoolExecutor, reducing Writer phase time by ~60%

• Logged all agent inputs, outputs, and latencies to SQLite enabling
  full pipeline observability across every run
```
...
- [ ] Live web search grounding via SerpAPI or Tavily
- [ ] LangGraph integration for formal agent graph management  
...
*Built by Dhruv — 1st year B.E. Information Science, NIE Mysuru*
